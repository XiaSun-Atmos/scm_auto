...
SPLIT_IC=os.environ['SPLIT_IC']
COMBINE_IC=os.environ['COMBINE_IC']
...
cyc=os.environ['cyc']
mmddcyc=mm+dd+cyc
mmddcyc2=mm+'_'+dd+'_'+cyc
mmddcyc3=mm+dd+'.'+cyc
CCPP_SUITE=os.environ['CCPP_SUITE']
FINAL_DIR=os.environ['CCPP_SUITE']
nlat_start,nlat_end=[0,200]
nlon_start,nlon_end=[0,220]
SCM_ROOT=os.environ['SCM_ROOT']
combine_ds=xr.open_dataset(COMBINE_IC+"/mumip_icon_2.5_IO_0.2_20160811.00_combined_fulladv.nc",drop_variables=["t0"])
combine_lat=combine_ds['lat']
combine_lon=combine_ds['lon']
combine_ds.close()
...
parser.add_argument('-j', '--jobs',       help='number of parallel jobs (default: number of CPU cores)', type=int, default=None)
parser.add_argument('--no-parallel',      help='disable parallel execution', action='store_true', default=False)
...
def parse_arguments():
    ...
    # Set default number of jobs to CPU count
    if jobs is None:
        jobs = cpu_count()
    ...
    return (file, case, sdf, namelist, tracers, gdb, runtime, runtime_mult, docker, \
            ...
            n_itt_diag, run_dir, bin_dir, timestep,jobs, no_parallel)
...
class Experiment(object):
    ...
    def __init__(self, case, suite, runtime, runtime_mult, levels, npz_type, vert_coord_file, case_data_dir, n_itt_out, n_itt_diag, timestep,nlat,nlon):
        ...
        if not os.path.isdir(os.path.join(SCM_ROOT, self._case_data_dir)):
            ...
            logging.critical(message)
            raise Exception(message)
       # Link SCM input data Xia
        if os.path.exists(os.path.join(SCM_ROOT, self._case_data_dir + '/' + self._case + '_SCM_driver.nc')):
            pass
        else:
            cmd = 'ln -s {0} {1}'.format(os.path.join(SPLIT_IC,f"{mmddcyc2}/lat_{nlat}/mumip_icon_2.5_IO_0.2_2016{mmddcyc3}_combined_lat{nlat}_lon{nlon}.nc"), os.path.join(SCM_ROOT, self._case_data_dir) + '/' + self._case + '_SCM_driver.nc')
            logging.debug('Linking SCM input data')
            execute(cmd)
            # print("Xia SCM_driver file exist")

        # Create case config nml file Xia
        logging.debug('SCM_ROOT = '+f"{SCM_ROOT}")
        shutil.copy(f"{SCRIPTS}/icon_cor_F0_template.nml", f"{SCM_ROOT}/scm/etc/case_config/icon_lat{nlat}_lon{nlon}_{mmddcyc}.nml")
        with open(f"{SCM_ROOT}/scm/etc/case_config/icon_lat{nlat}_lon{nlon}_{mmddcyc}.nml", 'r') as file:
            content = file.read()

        # Replace CASENAME with b (global replace)
        content = content.replace('CASENAME', f"icon_lat{nlat}_lon{nlon}_{mmddcyc}")
        # Replace OUTDIR with x (global replace)  
        content = content.replace('OUTDIR', f"output_icon_lat{nlat}_lon{nlon}_{mmddcyc}_{CCPP_SUITE}")

        with open(f"{SCM_ROOT}/scm/etc/case_config/icon_lat{nlat}_lon{nlon}_{mmddcyc}.nml", 'w') as file:
            file.write(content)
        file.close()                 
    ...
    def setup_rundir(self):
        ...
        print ("Xia: custom_output_dir is "+str(custom_output_dir))
        ...
        if(not custom_output_dir):
            ...
            print("Xia: entering question block")
            f90nml.patch(os.path.join(SCM_RUN, STANDARD_EXPERIMENT_NAMELIST), output_dir_patch_nml, 'temp_{0}_{1}.nml') #Xia
            cmd = "mv {0} {1}".format('temp_{0}_{1}.nml', os.path.join(SCM_RUN, STANDARD_EXPERIMENT_NAMELIST)) #Xia
            execute(cmd)
...
def launch_executable(use_gdb, gdb, scm_run, executable, ignore_error = False):
...
def run_single_case(args_tuple):
    """
    Function to run a single case - designed for multiprocessing
    args_tuple contains: (run_dict, process_id, global_args)
    """
    run, process_id, global_args = args_tuple
    
    # Unpack global arguments
    (runtime, runtime_mult, levels, npz_type, vert_coord_file, case_data_dir, 
     n_itt_out, n_itt_diag, timestep, use_gdb, gdb, docker, scm_root, 
     scm_bin, default_run_dir) = global_args

    # Check if already sucessfully ran
    print(f"processing {run["case"]}")
    # extract lat and lon from run["case"]
    match=re.search(r'lat(\d+)_lon(\d+)', f"{run["case"]}")
    if match:
        nlat = int(match.group(1))
        nlon = int(match.group(2))
    else:
        print("lat and lon not found")

    # check if SCM run already done, Xia

    if os.path.exists(f"{SCM_RESULTS}/{FINAL_DIR}/{mmddcyc2}/lat_{nlat}/mumip_icon_scm_2016{mmddcyc3}_lat{nlat}_lon{nlon}_{CCPP_SUITE}.nc") and os.path.getsize(f"{SCM_RESULTS}/{FINAL_DIR}/{mmddcyc2}/lat_{nlat}/mumip_icon_scm_2016{mmddcyc3}_lat{nlat}_lon{nlon}_{CCPP_SUITE}.nc") > 0:
        print(f"{FINAL_DIR} done for lat:"+str(nlat)+", lon:"+str(nlon))
    else:
       
        # Create unique run directory for this process
        # unique_run_dir = os.path.join(scm_root, default_run_dir + f'_proc_{process_id}')
        unique_run_dir = os.path.join(scm_root, default_run_dir + f'_{run["case"]}_{FINAL_DIR}')
        if not os.path.isdir(unique_run_dir):
            os.makedirs(unique_run_dir)
        
        executable = os.path.join(unique_run_dir, EXECUTABLE_NAME)
        print(executable)
        print(run)
        ...
        try:
            ...
            if active_suite:
                ...
                try:
                    ...
                    process_logger.info('Using provided namelist for suite={0}'.format(run["suite"]))
                except:
                    process_logger.info('Using default namelist for suite={0}'.format(run["suite"]))
                    
                # If tracer file provided, use. Otherwise use default tracer file for this suite
                try: 
                    active_suite.tracers = run["tracers"]
                    process_logger.info('Using provided tracer file for suite={0}'.format(run["suite"]))
                except:
                    process_logger.info('Using default tracer file for suite={0}'.format(run["suite"]))
            else:
                process_logger.info('Suite provided, {0}, does not have default namelist and tracer information'.format(run["suite"]))
                if ("namelist" in run) and ("tracers" in run):
                    ...
                    raise Exception('The given suite {0}, does not have defaults set in suite_info.py and either the tracers file or physics namelist file (or both) were not provided.'.format(run["suite"]))

            process_logger.info('Process {0}: Executing case={1}, suite={2}, namelist={3}'.format(
                process_id, run["case"], run["suite"], active_suite.namelist))
            
            # Create experiment with unique run directory
            exp = Experiment(run["case"], active_suite, runtime, runtime_mult, levels, 
                            npz_type, vert_coord_file, case_data_dir, n_itt_out, n_itt_diag, timestep,nlat,nlon)
            
            # Temporarily override global SCM_RUN for this process
            original_scm_run = globals()['SCM_RUN']
            globals()['SCM_RUN'] = unique_run_dir
            
            exp_dir = exp.setup_rundir()
            
            # # Restore original SCM_RUN
            # globals()['SCM_RUN'] = original_scm_run
            
            # Run the executable
            (status, time_elapsed) = launch_executable(use_gdb, gdb, unique_run_dir, executable, ignore_error=True)
            
            result = {
                'process_id': process_id,
                'case': run["case"],
                'suite': run["suite"],
                'namelist': active_suite.namelist,
                'status': status,
            ...
            if status == 0:
                process_logger.info('Process {0}: Case {1} completed successfully'.format(process_id, run["case"]))
                # === Post Prcessing : Assign new coordinates ===
                scm_file=f"{unique_run_dir}/output_icon_lat{nlat}_lon{nlon}_{mmddcyc}_{CCPP_SUITE}/output.nc"
                scm_ds=xr.open_dataset(scm_file)


                # === Remove conflicting scalar vars if needed ===
                ds_data = scm_ds.drop_vars(['lat', 'lon'], errors='ignore')
                ds_data=ds_data.squeeze(dim='hor_dim_layer')

                # === Prepare lat and lon dimension values===
                ic_lat=combine_lat[nlat]
                lat = xr.DataArray([ic_lat], dims=['lat'])
                ic_lon=combine_lon[nlon]
                lon = xr.DataArray([ic_lon], dims=['lon'])
                new_coords = {'lat': lat, 'lon': lon}

                # === Update variables ===
                new_vars = {}
                for var_name, var in ds_data.data_vars.items():
                    # print(var_name)
                    if var.ndim >= 1 and "time" not in var_name.lower():
                        # Broadcast to new dimensions (assume spatial dims are last)
                        core_dims = var.dims[:-2]
                        # expanded = var.expand_dims({'lat': ic_lat.shape[0], 'lon': ic_lon.shape[0]}, axis=(-2, -1))
                        expanded = var.expand_dims({'lat': 1, 'lon': 1}, axis=(-2, -1))
                        new_var = expanded.assign_coords(new_coords)
                        new_vars[var_name] = new_var
                    else:
                        new_vars[var_name] = var
                    # === Create new dataset ===
                ds_new = xr.Dataset(new_vars, coords={**scm_ds.coords, 'lat': lat, 'lon': lon})

                    # === Save to a new file ===
                ds_new.to_netcdf(f"{unique_run_dir}/output_icon_lat{nlat}_lon{nlon}_{mmddcyc}_{CCPP_SUITE}/output_latlon.nc")

                # Check if file exists and its size
                if os.path.exists(f"{unique_run_dir}/output_icon_lat{nlat}_lon{nlon}_{mmddcyc}_{CCPP_SUITE}/output_latlon.nc") and os.path.getsize(f"{unique_run_dir}/output_icon_lat{nlat}_lon{nlon}_{mmddcyc}_{CCPP_SUITE}/output_latlon.nc") == 0:
                    print(f"{nc_file} is empty. Doing something...")
                    ds_new.to_netcdf(f"{unique_run_dir}/output_icon_lat{nlat}_lon{nlon}_{mmddcyc}_{CCPP_SUITE}/output_latlon.nc")
                    # Do your action here, e.g., skip, delete, or log
                else:
                    print(f"{unique_run_dir}/output_icon_lat{nlat}_lon{nlon}_{mmddcyc}_{CCPP_SUITE}/output_latlon.nc has data. Proceed normally.")

                ds_new.close()
                scm_ds.close()
                print("Saved to 'output_latlon.nc'")
                target_dir=f"{SCM_RESULTS}/{FINAL_DIR}/{mmddcyc2}/lat_{nlat}"
                source_file=f"{unique_run_dir}/output_icon_lat{nlat}_lon{nlon}_{mmddcyc}_{CCPP_SUITE}/output_latlon.nc"
                # === Step 1: Make directory (like `mkdir -p`) ===
                os.makedirs(target_dir, exist_ok=True)  # does nothing if dir exists
                # === Step 2: Move file ===
                shutil.move(source_file, os.path.join(target_dir, f"mumip_icon_scm_2016{mmddcyc3}_lat{nlat}_lon{nlon}_{CCPP_SUITE}.nc"))

                # cleanning up
                os.remove(f"{SCM_ROOT}/scm/data/processed_case_input/icon_lat{nlat}_lon{nlon}_{mmddcyc}_SCM_driver.nc")
                os.remove(f"{SCM_ROOT}/scm/etc/case_config/icon_lat{nlat}_lon{nlon}_{mmddcyc}.nml")

            else:
                process_logger.warning('Process {0}: Case {1} exited with code {2}'.format(process_id, run["case"], status))
            
            if time_elapsed:
                process_logger.info('Process {0}: Elapsed time: {1}s'.format(process_id, time_elapsed))
            ...
            # Clean up unique run directory if desired
...
def main():
    (file, case, sdf, namelist, tracers, use_gdb, runtime, runtime_mult, docker, \
     verbose, levels, npz_type, vert_coord_file, case_data_dir, n_itt_out,       \
     n_itt_diag, run_dir, bin_dir, timestep, jobs, no_parallel) = parse_arguments()
    ...
    if (file != None):
        ...
        logging.info('SCM-run: Using generated array cases')
        run_list=[]
        for i in range(nlat_start,nlat_end):
            for j in range(nlon_start,nlon_end):
                run_config = {"case": f"icon_lat{i}_lon{j}_{mmddcyc}","suite": f"{CCPP_SUITE}"}
    ...
    # Prepare global arguments for multiprocessing
    global_args = (runtime, runtime_mult, levels, npz_type, vert_coord_file, case_data_dir, 
                   n_itt_out, n_itt_diag, timestep, use_gdb, gdb, docker, SCM_ROOT, 
                   SCM_BIN, DEFAULT_RUN_DIR)

    # Check if we should run in parallel
    if no_parallel or len(run_list) == 1 or use_gdb:
        # Run sequentially (original behavior)
        logging.info('Running cases sequentially')
        for i, run in enumerate(run_list):
            args_tuple = (run, i, global_args)
            result = run_single_case(args_tuple)
            logging.info('Completed case {0}: status={1}'.format(result['case'], result['status']))
    else:
        # Run in parallel
        logging.info('Running {0} cases in parallel using {1} processes'.format(len(run_list), jobs))
        
        # Prepare arguments for multiprocessing
        args_list = []
        for i, run in enumerate(run_list):
            args_list.append((run, i, global_args))
        print(args_list)
        
        # Create process pool and run cases in parallel
        with Pool(processes=jobs) as pool:
            results = pool.map(run_single_case, args_list)
        
        # Print summary of results
        logging.info('Parallel execution completed. Summary:')
        successful = 0
        failed = 0
        ...
        for result in results:
            if result and  result['status'] == 0:
                successful += 1
                logging.info('✓ Case {0}: SUCCESS (time: {1}s)'.format(
                    result['case'], result.get('time_elapsed', 'N/A')))
            else:
                failed += 1
                if result and 'error' in result:
                    logging.error('✗ Case {0}: FAILED - {1}'.format(result['case'], result['error']))
                elif result is not None:
                    logging.error('✗ Case {0}: FAILED (exit code: {1})'.format(
                        result['case'], result['status']))
                else:
                    print("Result is None!")
        
        logging.info('Total: {0} successful, {1} failed out of {2} cases'.format(
            successful, failed, len(run_list)))
