...
# rename vars
ncrename -h -v u_t,u -v v_t,v -v height_t,height -v pressure_t,pressure -v temp_t,temp \
-v qi_t,qi -v ql_t,ql -v qt_t,qt -v qv_t,qv -v theta_t,theta -v thetal_t,thetal \
init_time_${filename} -O init_time_${filename}
...
echo "rename time dependent variables"
ncrename -h -v ps,ps_forc -v height_t,height_forc -v pressure_t,pressure_forc -v u_t,u_nudging \
-v v_t,v_nudging -v theta_t,theta_nudging -v thetal_t,thetal_nudging -v temp_t,temp_forc \
-v qi_t,qi_nudging -v ql_t,ql_nudging -v qt_t,qt_nudging -v qv_t,qv_nudging -v rl_t,rl_nudging \
-v ri_t,ri_nudging -v rv_t,rv_nudging -v rt_t,rt_nudging \
${filename} -O ${filename}
...
ncatted -h -O -a units,t0,o,c,"seconds since ${datestring}" ${filename}  #Xia Sun, 03/2022
...
${filename} -O ${filename} 
# change time 
ncap2 -h -s 'time(:)={0,10800,21600,32400}' ${filename} -O ${filename}  #Xia Sun, 03/2022
echo "seconds since"

ncatted -h -O -a units,time,o,c,"seconds since ${datestring}" ${filename}
# add tke and set to 0
ncap2 -h -s 'tke[t0, lev, lat, lon]=0.' ${filename} -O ${filename}  #Xia Sun, 03/2022
