...
concat_scm (){
	...
	for ilon in  {0..219}; do
		rm -r ${CONCATE_OUTPUT}/concate_v2.0_${CCPP_SUITE}/${mm}_${dd}_${cyc}_output_lat$1/mumip_scm_${yyyy}$mm$dd.${cyc}_lat$1_lon${ilon}_v2.0_${CCPP_SUITE}.nc
	...
	cp -r $PYSCRIPTS/concat_lon_accum.py concat_lon_$2.py
	var=$2
	export var
	...
	module load nco
	ncrename -h  -v time_diag,time ${CCPP_SUITE}_ICON_${2}_$yyyy$mm$dd$cyc.nc -O ${CCPP_SUITE}_ICON_${2}_$yyyy$mm$dd$cyc.nc # rename time_inst var to time
...
parallel concat_scm ::: {0..199} ::: $(</home/xiasun/work2_xiasun/workflow_v1/scripts/vars_accum.txt)

for var in $(</home/xiasun/work2_xiasun/workflow_v1/scripts/vars_accum.txt)
do
	yyyy=$(echo $CDATE | cut -c1-4)
	mm=$(echo $CDATE | cut -c5-6)
	dd=$(echo $CDATE | cut -c7-8)
